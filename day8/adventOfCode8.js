...
var calculateCode = function() {
  for (var i = 0; i < input.length; i++) {
    var stringTotal = 0;
    var split = escape(input[i]).split(/(%..)/);
    for (var j = 0; j < split.length; j++) {
      if (split[j].match(/([A-Za-z])/)) {
        stringTotal += 1;
      } else if (split[j] == "\"") {
        stringTotal += 2;
      } else if (split[j] == "\\") {
    ...
    total.code += stringTotal + 2;
    console.log(split, stringTotal+2);
  }
  return total.code;
};
var calculateDoc = function() {
  for (var i = 0; i < input.length; i++) {
    var split = input[i].split('');
    total.doc += split.length;
  }
  return total.doc;
};
...
var run = function() {
  calculateCode();
  calculateDoc();
  return total.code - total.doc;
};

run();

console.log(total, total.code - total.doc, 6195 - total.doc);

// caluclateCode doesnt work properly because of ascii letter values
// Couldnt get second half figured out.
